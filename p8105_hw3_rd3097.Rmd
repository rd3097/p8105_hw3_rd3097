...
problem3_2 <- transform(problem3_1, education = as.numeric(education)) %>%
...
age_group_counts <- problem3_3_1 %>%
  group_by(age_groups,sex) %>%
  summarise(count = n())%>%
  ungroup(age_groups,sex)


plot1<- ggplot(age_group_counts, aes(x = age_groups, y = count, fill = sex)) +
